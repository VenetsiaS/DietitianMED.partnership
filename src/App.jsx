...
const KANBAN_ACTIVE = ["Нов","В преговори","Активна","На изчакване"];
...
const SEED = [
  {id:"1",brand:"NutriMax BG",category:"Хранителни добавки",contact:"Васил Петров",phone:"+359 88 123 4567",email:"vasil@nutrimax.bg",contactBirthday:"",contactNotes:"Предпочита комуникация по имейл",theyOffer:"Безплатни продукти + 15% комисионна",weOffer:"2 Instagram поста + 1 Reel/месец",collabType:"Афилиейт",status:"Активна",priority:"Висок",value:400,deadline:"2026-07-15",assignee:"Дери",notes:"Много добри условия.",dateAdded:"2026-04-01",links:"https://nutrimax.bg",tags:"дългосрочен",nextStep:"Изпрати отчет",nextStepDate:"2026-06-30",rating:5,fileName:null,fileData:null,history:[{user:"Дери",action:"създаде записа",at:"01.04.2026 09:00"}],comments:[],invoices:[],archived:false},
  {id:"2",brand:"FitFuel Bulgaria",category:"Спортна храна",contact:"Екатерина Николова",phone:"+359 87 765 4321",email:"e.nikolova@fitfuel.bg",contactBirthday:"",contactNotes:"",theyOffer:"200 €/пост + продукти",weOffer:"1 Reel + 3 Stories + блог обзор",collabType:"Платена реклама",status:"В преговори",priority:"Висок",value:300,deadline:"2026-06-05",assignee:"Миша",notes:"Чакаме финален договор.",dateAdded:"2026-04-15",links:"",tags:"сезонен",nextStep:"Финализирай договора",nextStepDate:"2026-06-01",rating:0,fileName:null,fileData:null,history:[{user:"Миша",action:"създаде записа",at:"15.04.2026 10:30"}],comments:[{author:"Миша",text:"Добър партньор, отзивчиви са. @Дери провери клаузата за ексклузивност.",at:"16.04.2026 09:00",mentions:["Дери"]}],invoices:[],archived:false},
  {id:"3",brand:"VitaPlus",category:"Фармация",contact:"Деница Иванова",phone:"+359 88 999 1234",email:"d.ivanova@vitaplus.bg",contactBirthday:"1985-03-15",contactNotes:"Запознахме се на HealthExpo 2025. Много приятна.",theyOffer:"Фиксиран хонорар 600€/месец",weOffer:"Амбасадор + 4 поста/месец",collabType:"Амбасадор",status:"Активна",priority:"Висок",value:600,deadline:"2026-12-31",assignee:"Дери",notes:"Топ партньор.",dateAdded:"2026-03-01",links:"https://vitaplus.bg",tags:"дългосрочен,амбасадор",nextStep:"Месечен отчет",nextStepDate:"2026-06-30",rating:5,fileName:null,fileData:null,history:[{user:"Дери",action:"създаде записа",at:"01.03.2026 08:00"}],comments:[{author:"Миша",text:"Отлични условия! @Дери можеш ли да провериш договора?",at:"02.03.2026 11:00",mentions:["Дери"]}],invoices:[{id:"inv1",number:"2026-001",amount:600,currency:"€",issueDate:"2026-04-01",dueDate:"2026-04-15",status:"Платена",notes:"Април 2026",paidDate:"2026-04-14"},{id:"inv2",number:"2026-002",amount:600,currency:"€",issueDate:"2026-05-01",dueDate:"2026-05-15",status:"Изпратена",notes:"Май 2026",paidDate:""}],archived:false},
  {id:"4",brand:"HealthyLife",category:"Козметика",contact:"Борис Стоянов",phone:"+359 89 234 5678",email:"boris@healthylife.bg",contactBirthday:"",contactNotes:"",theyOffer:"Бартер — продукти срещу съдържание",weOffer:"2 поста + story mention",collabType:"Бартер",status:"Отказана",priority:"Нисък",value:0,deadline:"",assignee:"Вени",notes:"Условията не бяха подходящи.",dateAdded:"2026-02-01",links:"",tags:"",nextStep:"",nextStepDate:"",rating:2,fileName:null,fileData:null,history:[{user:"Вени",action:"създаде записа",at:"01.02.2026 10:00"},{user:"Вени",action:"промени статус → Отказана",at:"15.02.2026 14:00"}],comments:[],invoices:[],archived:true},
];
...
function Badge({status,color}){
  const dot=color||DEF_ST_COLORS[status]||"#94a3b8";
  const bg=dot+"22"; const text=dot;
  return <span style={{background:bg,color:text,borderRadius:20,padding:"3px 9px",fontSize:11,fontWeight:600,display:"inline-flex",alignItems:"center",gap:4,whiteSpace:"nowrap"}}><span style={{width:6,height:6,borderRadius:"50%",background:dot,flexShrink:0}}/>{status}</span>;
}
...
function MentionInput({value,onChange,placeholder}){
  ...
  const suggestions=TEAM.filter(m=>m.toLowerCase().startsWith(q.toLowerCase()));
  return <div style={{position:"relative",flex:1}}><textarea ref={ref} style={{...IS,resize:"none",height:54,width:"100%"}} placeholder={placeholder} value={value} onChange={handleChange} onKeyDown={e=>{if(e.key==="Escape") setShow(false);}}/>{show&&suggestions.length>0&&<div style={{position:"absolute",bottom:"100%",left:0,background:"#fff",border:"1.5px solid #d1fae5",borderRadius:8,boxShadow:"0 4px 16px rgba(0,56,63,.15)",zIndex:100,overflow:"hidden",marginBottom:4}}>{suggestions.map(m=><div key={m} onClick={()=>pick(m)} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",cursor:"pointer",fontSize:13,color:"#00383f"}} onMouseEnter={e=>e.currentTarget.style.background="#e6f7f2"} onMouseLeave={e=>e.currentTarget.style.background=""}><Av name={m} size={22}/>{m}</div>)}</div>}</div>;
}
...
function StatusSettings({statuses,onSave,onClose}){
  ...
  const add=()=>setList(p=>[...p,{name:"Нов статус",color:"#6366f1",isArchive:false}]);
  const upd=(i,k,v)=>setList(p=>p.map((s,j)=>j===i?{...s,[k]:v}:s));
  const del=i=>setList(p=>p.filter((_,j)=>j!==i));
  return <div style={{padding:24}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18}}>
      <h3 style={{margin:0,fontSize:17,fontWeight:800,color:"#00383f"}}>⚙️ Персонализирани статуси</h3>
      <button onClick={onClose} style={{background:"none",border:"none",fontSize:20,cursor:"pointer",color:"#64748b"}}>×</button>
    </div>
    {list.map((s,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
      <input type="color" value={s.color} onChange={e=>upd(i,"color",e.target.value)} style={{width:32,height:32,border:"1.5px solid #d1fae5",borderRadius:6,cursor:"pointer",padding:2}}/>
      <input style={{...IS,flex:1}} value={s.name} onChange={e=>upd(i,"name",e.target.value)}/>
      <label style={{display:"flex",alignItems:"center",gap:5,fontSize:12,color:"#64748b",whiteSpace:"nowrap"}}>
        <input type="checkbox" checked={!!s.isArchive} onChange={e=>upd(i,"isArchive",e.target.checked)}/> Архивира
      </label>
      <button onClick={()=>del(i)} style={{background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontSize:16,flexShrink:0}}>🗑️</button>
    </div>)}
    <button onClick={add} style={{padding:"7px 14px",borderRadius:8,border:"1.5px dashed #02a576",background:"#f8fffe",color:"#02a576",cursor:"pointer",fontSize:12,fontFamily:"inherit",marginBottom:16}}>+ Добави статус</button>
...
function InvoiceTab({item,onUpdate,notify}){
  ...
  const paid=invoices.filter(i=>i.status==="Платена").reduce((s,i)=>s+(Number(i.amount)||0),0);
  const openNew=()=>{setF({...EMPTY_INV,number:`INV-${Date.now().toString().slice(-5)}`});setEditInv(null);setShowForm(true);};
  const openEdit=inv=>{setF({...inv});setEditInv(inv.id);setShowForm(true);};
  ...
  const del=id=>{onUpdate(invoices.filter(i=>i.id!==id));notify("🗑️ Изтрита");};
  return <div>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
      <div style={{display:"flex",gap:14}}>
        {[["Общо",total,"#00383f"],["Платено",paid,"#22c55e"],["Чакащо",total-paid,"#f59e0b"]].map(([l,v,c])=><div key={l}><div style={{fontSize:10,fontWeight:700,color:"#02a576",textTransform:"uppercase",letterSpacing:".07em"}}>{l}</div><div style={{fontSize:18,fontWeight:800,color:c}}>€{v}</div></div>)}
      </div>
      <button onClick={openNew} style={{padding:"7px 14px",borderRadius:8,border:"none",background:"#02a576",color:"#fff",cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:"inherit"}}>+ Фактура</button>
    </div>
    ...
    {invoices.map(inv=>{const overdue=inv.status!=="Платена"&&inv.dueDate&&dl(inv.dueDate)<0; return <div key={inv.id} style={{background:"#f8fffe",borderRadius:9,padding:"11px 13px",marginBottom:8,border:`1.5px solid ${overdue?"#fca5a5":"#d1fae5"}`}}>
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:6}}>
        <div><span style={{fontSize:13,fontWeight:700,color:"#00383f"}}>{inv.number}</span>{inv.notes&&<span style={{fontSize:11,color:"#64748b",marginLeft:8}}>{inv.notes}</span>}</div>
        <div style={{display:"flex",gap:6,alignItems:"center"}}><InvBadge status={overdue?"Просрочена":inv.status}/><button onClick={()=>openEdit(inv)} style={{background:"none",border:"none",cursor:"pointer",fontSize:13,padding:"0 2px"}}>✏️</button><button onClick={()=>del(inv.id)} style={{background:"none",border:"none",cursor:"pointer",fontSize:13,color:"#ef4444",padding:"0 2px"}}>🗑️</button></div>
      </div>
    ...
    {showForm&&<div style={{background:"#f0fdf9",borderRadius:10,padding:14,border:"1.5px solid #d1fae5",marginTop:12}}>
      ...
      <FF label="Бележки"><input style={IS} value={f.notes} onChange={e=>setF(p=>({...p,notes:e.target.value}))} placeholder="Април 2026..."/></FF>
...
function MonthlySummary({items}){
  const months=useMemo(()=>{
    const map={};
    items.forEach(item=>{
      const m=getMonth(item.dateAdded); if(!m) return;
      if(!map[m]) map[m]={month:m,dateKey:item.dateAdded,new:0,activated:0,invoiced:0,paid:0,negotiations:0};
      map[m].new++;
      if(item.status==="Активна") map[m].activated++;
      if(item.status==="В преговори") map[m].negotiations++;
      (item.invoices||[]).forEach(inv=>{map[m].invoiced+=(Number(inv.amount)||0); if(inv.status==="Платена") map[m].paid+=(Number(inv.amount)||0);});
    });
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:12}}>
    {months.map(m=><div key={m.month} style={{background:"#f8fffe",borderRadius:12,padding:"16px 18px",border:"1.5px solid #d1fae5"}}>
      <div style={{fontSize:14,fontWeight:800,color:"#00383f",marginBottom:12,textTransform:"capitalize"}}>{m.month}</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(100px,1fr))",gap:10}}>
        {[["🆕 Нови",m.new,"#6366f1"],["✅ Активирани",m.activated,"#22c55e"],["🔄 Преговори",m.negotiations,"#eab308"],["🧾 Фактурирано",`€${m.invoiced}`,"#02a576"],["💶 Получено",`€${m.paid}`,"#00383f"]].map(([l,v,c])=><div key={l} style={{background:"#fff",borderRadius:8,padding:"10px 12px",border:"1.5px solid #e6f7f2"}}><div style={{fontSize:10,color:"#64748b",marginBottom:3}}>{l}</div><div style={{fontSize:18,fontWeight:800,color:c}}>{v}</div></div>)}
      </div>
    </div>)}
  </div>;
}
...
function QuarterlyComparison({items}){
  const quarters=useMemo(()=>{
    const map={};
    items.forEach(item=>{
      const q=getQuarter(item.dateAdded); if(!q) return;
      if(!map[q]) map[q]={q,dateKey:item.dateAdded,total:0,active:0,value:0,invoiced:0,paid:0};
      map[q].total++; if(item.status==="Активна"){map[q].active++; map[q].value+=(Number(item.value)||0);}
      (item.invoices||[]).forEach(inv=>{map[q].invoiced+=(Number(inv.amount)||0); if(inv.status==="Платена") map[q].paid+=(Number(inv.amount)||0);});
    });
    return Object.values(map).sort((a,b)=>new Date(a.dateKey)-new Date(b.dateKey));
  },[items]);

  if(quarters.length<1) return <div style={{color:"#94a3b8",fontSize:13,textAlign:"center",padding:32}}>Няма достатъчно данни</div>;
  const maxVal=Math.max(...quarters.map(q=>q.invoiced),1);
  return <div>
    <div style={{display:"grid",gridTemplateColumns:`repeat(${Math.min(quarters.length,4)},1fr)`,gap:12,marginBottom:20}}>
      {quarters.map((q,i)=>{
        const prev=quarters[i-1];
        const diff=prev?Math.round(((q.invoiced-prev.invoiced)/Math.max(prev.invoiced,1))*100):null;
        return <div key={q.q} style={{background:"#fff",borderRadius:12,padding:"14px 16px",border:"1.5px solid #d1fae5"}}>
          <div style={{fontSize:12,fontWeight:800,color:"#00383f",marginBottom:10}}>{q.q}</div>
          <div style={{height:6,background:"#e6f7f2",borderRadius:4,marginBottom:10}}><div style={{width:`${(q.invoiced/maxVal)*100}%`,height:"100%",background:"#02a576",borderRadius:4}}/></div>
          {[["Общо",q.total],["Активни",q.active],["Стойност",`€${q.value}/мес`],["Фактурирано",`€${q.invoiced}`],["Получено",`€${q.paid}`]].map(([l,v])=><div key={l} style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}}><span style={{color:"#64748b"}}>{l}</span><span style={{fontWeight:700,color:"#00383f"}}>{v}</span></div>)}
          {diff!==null&&<div style={{marginTop:8,fontSize:11,fontWeight:700,color:diff>=0?"#22c55e":"#ef4444",textAlign:"right"}}>{diff>=0?"↑":"↓"} {Math.abs(diff)}% vs {prev.q}</div>}
        </div>;
      })}
    </div>
    {quarters.length>=2&&<div style={{background:"#f8fffe",borderRadius:10,padding:"12px 16px",border:"1.5px solid #d1fae5",fontSize:12,color:"#00383f"}}>
      <b>Сравнение последни 2 тримесечия:</b> {quarters[quarters.length-2].q} → {quarters[quarters.length-1].q} &nbsp;|&nbsp; Фактурирано: €{quarters[quarters.length-2].invoiced} → €{quarters[quarters.length-1].invoiced} &nbsp;|&nbsp; Колаборации: {quarters[quarters.length-2].total} → {quarters[quarters.length-1].total}
    </div>}
  </div>;
}

// ── Brand History ────────────────────────────────────────────────────────────
function BrandHistory({brand,items}){
  const hist=items.filter(i=>i.brand.toLowerCase()===brand.toLowerCase());
  if(hist.length<=1) return <div style={{color:"#94a3b8",fontSize:13}}>Само една колаборация с този бранд</div>;
  return <div style={{display:"flex",flexDirection:"column",gap:10}}>
    <div style={{fontSize:12,color:"#64748b",marginBottom:4}}>Намерени {hist.length} колаборации с <b>{brand}</b></div>
    {hist.map(i=><div key={i.id} style={{background:"#f8fffe",borderRadius:9,padding:"11px 14px",border:"1.5px solid #d1fae5"}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontWeight:700,color:"#00383f",fontSize:13}}>{i.collabType||"Колаборация"}</span><Badge status={i.status} color={DEF_ST_COLORS[i.status]}/></div>
      <div style={{fontSize:12,color:"#64748b"}}>Добавена: {fmt(i.dateAdded)} · {i.value?`€${i.value}/мес`:"бартер"} · {i.assignee||"—"}</div>
      {i.invoices?.length>0&&<div style={{fontSize:12,color:"#02a576",marginTop:3}}>🧾 {i.invoices.length} фактури · €{i.invoices.reduce((s,x)=>s+(Number(x.amount)||0),0)} общо</div>}
    </div>)}
  </div>;
}
...
function Dashboard({items,statuses}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    {topCat.length>0&&<div style={{background:"#fff",borderRadius:12,padding:18,border:"1.5px solid #d1fae5"}}><div style={{fontSize:13,fontWeight:700,color:"#00383f",marginBottom:14}}>Топ приходи по категория</div>{topCat.map(c=><div key={c.n} style={{display:"flex",alignItems:"center",gap:8,marginBottom:9}}><div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:"#00383f",marginBottom:2}}>{c.n}</div><div style={{height:6,background:"#e6f7f2",borderRadius:4}}><div style={{width:`${(c.v/topCat[0].v)*100}%`,height:"100%",background:"#02a576",borderRadius:4}}/></div></div><span style={{fontSize:13,fontWeight:700,color:"#02a576",minWidth:50,textAlign:"right"}}>€{c.v}</span></div>)}</div>}
  </div>;
}

// ── Kanban with Drag & Drop + Sort ───────────────────────────────────────────
function Kanban({items,statuses,onOpen,onStatusChange}){
  ...
  const sortedItems=(col)=>{
    const ci=items.filter(i=>i.status===col&&!i.archived);
    return [...ci].sort((a,b)=>{let va=a[sortK]??"",vb=b[sortK]??""; if(sortK==="value"||sortK==="rating"){va=Number(va);vb=Number(vb);} const c=va<vb?-1:va>vb?1:0; return sortD==="asc"?c:-c;});
  };
  return <div>
    <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:12,flexWrap:"wrap"}}>
      <span style={{fontSize:12,color:"#64748b",fontWeight:600}}>Сортирай по:</span>
      {[["brand","Бранд"],["priority","Приоритет"],["value","Стойност"],["deadline","Deadline"]].map(([k,l])=><button key={k} onClick={()=>{if(sortK===k) setSortD(d=>d==="asc"?"desc":"asc"); else{setSortK(k);setSortD("asc");}}} style={{padding:"5px 10px",borderRadius:7,border:"1.5px solid",borderColor:sortK===k?"#02a576":"#d1fae5",background:sortK===k?"#e6f7f2":"#fff",color:sortK===k?"#00383f":"#64748b",cursor:"pointer",fontSize:11,fontFamily:"inherit",fontWeight:sortK===k?700:400}}>{l}{sortK===k?(sortD==="asc"?" ↑":" ↓"):""}</button>)}
    </div>
    <div style={{display:"flex",gap:12,overflowX:"auto",paddingBottom:14,alignItems:"flex-start"}}>
      {activeCols.map(col=>{
        const ci=sortedItems(col); const s=statuses.find(x=>x.name===col)||{}; const isOver=over===col;
        return <div key={col} onDragOver={e=>{e.preventDefault();setOver(col);}} onDrop={e=>{e.preventDefault();if(dragging&&col!==items.find(i=>i.id===dragging)?.status) onStatusChange(dragging,col);setDragging(null);setOver(null);}} onDragLeave={()=>setOver(null)} style={{minWidth:210,flex:"0 0 210px",background:isOver?"#e6f7f2":"#f8fffe",borderRadius:12,padding:"12px 10px",border:`2px ${isOver?"solid #02a576":"dashed #d1fae5"}`,transition:"all .15s",minHeight:120}}>
          ...
          {ci.map(item=>{const isDrag=dragging===item.id; return <div key={item.id} draggable onDragStart={e=>{setDragging(item.id);e.dataTransfer.effectAllowed="move";}} onDragEnd={()=>{setDragging(null);setOver(null);}} onClick={()=>!dragging&&onOpen(item)} style={{background:"#fff",borderRadius:10,padding:"11px 12px",border:`1.5px solid ${isDrag?"#02a576":"#d1fae5"}`,cursor:"grab",boxShadow:isDrag?"0 8px 20px rgba(2,165,118,.2)":"0 1px 3px rgba(0,56,63,.07)",transform:isDrag?"rotate(2deg) scale(1.02)":"none",opacity:isDrag?.7:1,marginBottom:8,transition:"transform .1s,box-shadow .1s",userSelect:"none"}}>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:4}}>
              ...
              <div style={{display:"flex",gap:5,alignItems:"center"}}>
                {item.invoices?.length>0&&<span style={{fontSize:10,color:"#02a576"}}>🧾{item.invoices.length}</span>}
                {item.comments?.length>0&&<span style={{fontSize:10,color:"#64748b"}}>💬{item.comments.length}</span>}
                {item.rating>0&&<span style={{fontSize:10,color:"#f59e0b"}}>{"★".repeat(item.rating)}</span>}
              </div>
            </div>
          </div>;})}
...
function Detail({item,onClose,onEdit,onDelete,onComment,onDuplicate,onInvoiceUpdate,onArchive,notify,allItems,statuses}){
  ...
  const tabs=[["info","ℹ️ Детайли"],[`invoices`,`🧾 Фактури${item.invoices?.length?` (${item.invoices.length})`:""}`],[`comments`,`💬 Коментари${item.comments?.length?` (${item.comments.length})`:""}`],["history","📋 История"],["brandhistory","🔁 Бранд"]];
  ...
  return <div style={{padding:26}}>
    ...
    <div style={{display:"flex",gap:2,marginBottom:16,borderBottom:"2px solid #e6f7f2",overflowX:"auto"}}>
      {tabs.map(([k,l])=><button key={k} onClick={()=>setTab(k)} style={{padding:"7px 11px",border:"none",borderRadius:"7px 7px 0 0",background:tab===k?"#e6f7f2":"transparent",color:tab===k?"#00383f":"#64748b",fontWeight:tab===k?700:500,cursor:"pointer",fontSize:11,fontFamily:"inherit",whiteSpace:"nowrap"}}>{l}</button>)}
    </div>
    {tab==="info"&&<>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"11px 22px",marginBottom:16}}>
        {[["КАТЕГОРИЯ",item.category],["КОНТАКТ",item.contact],["ИМЕЙЛ",item.email],["ТЕЛЕФОН",item.phone],["ПРИОРИТЕТ",<span style={{color:PC[item.priority],fontWeight:700}}>● {item.priority}</span>],["ОТГОВОРНИК",item.assignee],["СТОЙНОСТ",item.value?`€${item.value}/мес`:"—"],["ТИП",item.collabType],["DEADLINE",item.deadline?<span style={{color:d!==null&&d<7&&d>=0?"#ef4444":"#00383f"}}>{fmt(item.deadline)}{d!==null&&d>=0?` (${d} дни)`:d<0?" (изтекъл)":""}</span>:"—"],["ДОБАВЕНО",fmt(item.dateAdded)]].map(([l,v])=><div key={l}><div style={{fontSize:9,fontWeight:700,color:"#02a576",letterSpacing:".07em",textTransform:"uppercase",marginBottom:2}}>{l}</div><div style={{fontSize:13,color:"#00383f"}}>{v||"—"}</div></div>)}
      </div>
      {(item.contactBirthday||item.contactNotes)&&<div style={{background:"#f0fdf9",borderRadius:9,padding:"11px 13px",marginBottom:10}}>
        <div style={{fontSize:9,fontWeight:700,color:"#02a576",textTransform:"uppercase",letterSpacing:".07em",marginBottom:6}}>БЕЛЕЖКИ ЗА КОНТАКТА</div>
        {item.contactBirthday&&<div style={{fontSize:12,color:"#00383f",marginBottom:4}}>🎂 Рожден ден: {fmt(item.contactBirthday)}</div>}
        {item.contactNotes&&<div style={{fontSize:13,color:"#475569"}}>{item.contactNotes}</div>}
      </div>}
      ...
      {item.links&&<div style={{marginBottom:8}}><div style={{fontSize:9,fontWeight:700,color:"#02a576",textTransform:"uppercase",letterSpacing:".07em",marginBottom:3}}>ЛИНКОВЕ</div>{item.links.split(",").map(l=>l.trim()).filter(Boolean).map((l,i)=><a key={i} href={l} target="_blank" rel="noopener" style={{display:"block",fontSize:13,color:"#02a576",textDecoration:"none",marginBottom:2}}>🔗 {l}</a>)}</div>}
    ...
    {tab==="comments"&&<div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:14}}>
        {[...(item.comments||[])].reverse().map((c,i)=><div key={i} style={{background:"#f8fffe",borderRadius:9,padding:"9px 12px",border:`1.5px solid ${c.mentions?.length?"#d1fae5":"#f0fdf9"}`}}>
          <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:3}}><Av name={c.author} size={20}/><span style={{fontSize:12,fontWeight:700,color:"#00383f"}}>{c.author}</span>{c.mentions?.length>0&&<span style={{fontSize:10,background:"#e6f7f2",color:"#02a576",borderRadius:4,padding:"1px 5px",fontWeight:600}}>@ {c.mentions.join(", ")}</span>}<span style={{fontSize:10,color:"#94a3b8",marginLeft:"auto"}}>{c.at}</span></div>
          <CommentText text={c.text}/>
        </div>)}
      </div>
      <div style={{display:"flex",gap:7,alignItems:"flex-end"}}>
        <select style={{...IS,width:110,flexShrink:0}} value={who} onChange={e=>setWho(e.target.value)}>{TEAM.map(m=><option key={m}>{m}</option>)}</select>
        <MentionInput value={txt} onChange={setTxt} placeholder="Пиши @ за да споменеш някого..."/>
        <button onClick={submitComment} style={{padding:"8px 14px",borderRadius:8,border:"none",background:"#02a576",color:"#fff",cursor:"pointer",fontFamily:"inherit",fontWeight:600,fontSize:12,height:54,flexShrink:0}}>Добави</button>
      </div>
    </div>}
    {tab==="history"&&<div>
      {!(item.history?.length)&&<div style={{color:"#94a3b8",fontSize:13}}>Няма история</div>}
      {[...(item.history||[])].reverse().map((h,i)=><div key={i} style={{display:"flex",gap:10,padding:"8px 0",borderBottom:"1px solid #e6f7f2"}}><span style={{width:6,height:6,borderRadius:"50%",background:"#02a576",marginTop:5,flexShrink:0}}/><div><div style={{fontSize:13,color:"#00383f"}}>{h.user&&<b>{h.user} </b>}{h.action}{h.detail?`: ${h.detail}`:""}</div><div style={{fontSize:10,color:"#94a3b8",marginTop:1}}>{h.at}</div></div></div>)}
    </div>}
    {tab==="brandhistory"&&<BrandHistory brand={item.brand} items={allItems}/>}
  </div>;
}
...
function Form({initial,onSave,onCancel,statuses}){
  ...
  const ok=f.brand.trim()&&f.contact.trim()&&f.email.trim();
  return <div style={{padding:24}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 14px"}}>
      ...
      <FF label="Бележки за контакта"><input style={IS} value={f.contactNotes} onChange={e=>set("contactNotes",e.target.value)} placeholder="Как се запознахте, предпочитания..."/></FF>
    </div>
    ...
    <FF label="Прикачи файл">
      <div style={{display:"flex",alignItems:"center",gap:9}}>
        <button onClick={()=>fileRef.current.click()} style={{padding:"6px 14px",borderRadius:8,border:"1.5px dashed #02a576",background:"#f8fffe",cursor:"pointer",fontSize:12,color:"#02a576",fontFamily:"inherit"}}>📎 {f.fileName||"Избери файл"}</button>
        {f.fileName&&<button onClick={()=>{set("fileName",null);set("fileData",null);}} style={{background:"none",border:"none",cursor:"pointer",color:"#ef4444",fontSize:17}}>×</button>}
      </div>
      <input ref={fileRef} type="file" style={{display:"none"}} onChange={e=>{const file=e.target.files[0]; if(!file) return; const r=new FileReader(); r.onload=ev=>{set("fileName",file.name);set("fileData",ev.target.result);}; r.readAsDataURL(file);}} accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg"/>
    </FF>
...
function printReport(items,statuses){
  ...
  const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>DietitianMED Partnerships Report</title><style>body{font-family:'Segoe UI',sans-serif;color:#00383f;padding:32px;max-width:900px;margin:0 auto} h1{font-size:24px;font-weight:800;color:#00383f;margin-bottom:4px} .sub{color:#64748b;margin-bottom:24px;font-size:14px} .stats{display:flex;gap:16px;margin-bottom:28px;flex-wrap:wrap} .stat{background:#f0fdf9;border:1.5px solid #d1fae5;border-radius:10px;padding:12px 18px;min-width:120px} .stat-l{font-size:10px;color:#02a576;font-weight:700;text-transform:uppercase;letter-spacing:.06em;margin-bottom:3px} .stat-v{font-size:22px;font-weight:800;color:#00383f} table{width:100%;border-collapse:collapse;font-size:13px} th{padding:10px;text-align:left;font-size:10px;font-weight:700;color:#02a576;text-transform:uppercase;letter-spacing:.06em;border-bottom:2px solid #d1fae5;background:#f8fffe} @media print{body{padding:16px}}</style></head><body>
  <h1>🤝 DietitianMED Partnerships</h1>
  <div class="sub">Генериран: ${new Date().toLocaleDateString("bg-BG",{day:"numeric",month:"long",year:"numeric"})}</div>
  <div class="stats">
    <div class="stat"><div class="stat-l">Общо</div><div class="stat-v">${items.filter(i=>!i.archived).length}</div></div>
    <div class="stat"><div class="stat-l">Активни</div><div class="stat-v">${items.filter(i=>i.status==="Активна"&&!i.archived).length}</div></div>
    <div class="stat"><div class="stat-l">Приходи/мес</div><div class="stat-v">€${totalRev}</div></div>
    <div class="stat"><div class="stat-l">Фактурирано</div><div class="stat-v">€${totalInv}</div></div>
    <div class="stat"><div class="stat-l">Получено</div><div class="stat-v">€${paidInv}</div></div>
  </div>
  <table><thead><tr><th>Бранд</th><th>Категория</th><th>Статус</th><th>Стойност</th><th>Отговорник</th><th>Deadline</th></tr></thead><tbody>${rows}</tbody></table>
  </body></html>`;
...
function BrandGroupView({items, statuses, onOpen}){
  ...
  const groups = useMemo(()=>{
    ...
    items.forEach(item=>{
      ...
      if(!map[key]) map[key] = {name:item.brand, items:[]};
  ...
  return <div>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
      <input style={{padding:"8px 12px",borderRadius:8,border:"1.5px solid #d1fae5",fontSize:13,color:"#00383f",background:"#f8fffe",outline:"none",fontFamily:"inherit",width:240}} placeholder="🔍 Търси бранд..." value={search} onChange={e=>setSearch(e.target.value)}/>
      <span style={{fontSize:12,color:"#02a576",fontWeight:600}}>{groups.length} бранда · {items.length} колаборации общо</span>
    </div>
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      {groups.map(g=>{
        const key = g.name.toLowerCase().trim();
        const isOpen = expanded[key];
        const totalVal = g.items.filter(i=>i.status==="Активна").reduce((s,i)=>s+(Number(i.value)||0),0);
        const totalInv = g.items.flatMap(i=>i.invoices||[]).reduce((s,i)=>s+(Number(i.amount)||0),0);
        const statCounts = {};
        g.items.forEach(i=>{ statCounts[i.status]=(statCounts[i.status]||0)+1; });
        return <div key={key} style={{background:"#fff",borderRadius:12,border:"1.5px solid #d1fae5",overflow:"hidden"}}>
          <div onClick={()=>toggle(key)} style={{display:"flex",alignItems:"center",gap:12,padding:"14px 16px",cursor:"pointer",userSelect:"none"}} onMouseEnter={e=>e.currentTarget.style.background="#f8fffe"} onMouseLeave={e=>e.currentTarget.style.background=""}>
            <Av name={g.name} size={38}/>
            <div style={{flex:1}}>
              <div style={{fontSize:14,fontWeight:800,color:"#00383f",marginBottom:4}}>{g.name}</div>
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                {Object.entries(statCounts).map(([st,n])=>{
                  const c = statuses.find(s=>s.name===st)?.color||DEF_ST_COLORS[st]||"#94a3b8";
                  return <span key={st} style={{background:c+"22",color:c,borderRadius:10,padding:"1px 7px",fontSize:11,fontWeight:600}}>{st} {n}</span>;
                })}
              </div>
            </div>
            <div style={{display:"flex",gap:16,alignItems:"center",flexShrink:0}}>
              {totalVal>0&&<div style={{textAlign:"right"}}><div style={{fontSize:9,color:"#02a576",fontWeight:700,textTransform:"uppercase",letterSpacing:".06em"}}>Активни €/мес</div><div style={{fontSize:15,fontWeight:800,color:"#02a576"}}>€{totalVal}</div></div>}
          ...
          {isOpen&&<div style={{borderTop:"1.5px solid #e6f7f2"}}>
            {g.items.map((item,idx)=>{
              const d=dl(item.deadline);
              const stColor=statuses.find(s=>s.name===item.status)?.color||DEF_ST_COLORS[item.status]||"#94a3b8";
              return <div key={item.id} onClick={()=>onOpen(item)} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 16px",borderBottom:idx<g.items.length-1?"1px solid #f0fdf9":"none",cursor:"pointer",transition:"background .1s"}} onMouseEnter={e=>e.currentTarget.style.background="#f8fffe"} onMouseLeave={e=>e.currentTarget.style.background=""}>
                <div style={{width:6,height:6,borderRadius:"50%",background:stColor,flexShrink:0}}/>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                    <span style={{fontSize:13,fontWeight:600,color:"#00383f"}}>{item.collabType||"Колаборация"}</span>
                    <Badge status={item.status} color={stColor}/>
                    {item.archived&&<span style={{fontSize:9,background:"#f1f5f9",color:"#64748b",borderRadius:4,padding:"1px 5px"}}>архив</span>}
                  </div>
                  <div style={{fontSize:11,color:"#64748b",marginTop:3}}>{item.theyOffer?.slice(0,70)}{item.theyOffer?.length>70?"…":""}</div>
                </div>
                <div style={{display:"flex",gap:14,alignItems:"center",flexShrink:0,fontSize:12}}>
                  {item.value>0&&<span style={{fontWeight:700,color:"#02a576"}}>€{item.value}/мес</span>}
                  {item.deadline&&<span style={{color:d!==null&&d<7&&d>=0?"#ef4444":"#64748b"}}>{new Date(item.deadline).toLocaleDateString("bg-BG")}</span>}
                  {item.assignee&&<Av name={item.assignee} size={22}/>}
                  {item.rating>0&&<span style={{color:"#f59e0b",fontSize:11}}>{"★".repeat(item.rating)}</span>}
                </div>
              </div>;
            })}
          </div>}
      ...
      {!groups.length&&<div style={{textAlign:"center",color:"#94a3b8",padding:40,fontSize:14}}>Няма намерени брандове</div>}
    </div>
  </div>;
}
...
function AttachmentsGallery({items, onOpen}){
  ...
  const allFiles = useMemo(()=>{
    ...
    return files.filter(f=>!search||f.name.toLowerCase().includes(search.toLowerCase())||f.brand.toLowerCase().includes(search.toLowerCase()))
      .sort((a,b)=>new Date(b.addedAt)-new Date(a.addedAt));
  },[items, search]);

  const isImage = type => ["jpg","jpeg","png","gif","webp"].includes(type);
  const isPdf = type => type==="pdf";
  const icon = type => isPdf(type)?"📄":isImage(type)?"🖼️":"📎";

  return <div>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
      <input style={{padding:"8px 12px",borderRadius:8,border:"1.5px solid #d1fae5",fontSize:13,color:"#00383f",background:"#f8fffe",outline:"none",fontFamily:"inherit",width:240}} placeholder="🔍 Търси файл или бранд..." value={search} onChange={e=>setSearch(e.target.value)}/>
      <span style={{fontSize:12,color:"#02a576",fontWeight:600}}>{allFiles.length} файла</span>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:12}}>
      {allFiles.map(f=><div key={f.id} style={{background:"#fff",borderRadius:12,border:"1.5px solid #d1fae5",overflow:"hidden",cursor:"pointer",transition:"all .15s",boxShadow:"0 1px 3px rgba(0,56,63,.06)"}} onMouseEnter={e=>{e.currentTarget.style.boxShadow="0 4px 12px rgba(2,165,118,.15)";e.currentTarget.style.transform="translateY(-2px)";}} onMouseLeave={e=>{e.currentTarget.style.boxShadow="0 1px 3px rgba(0,56,63,.06)";e.currentTarget.style.transform="";}}>
        {/* preview area */}
        <div onClick={()=>setPreview(f)} style={{height:110,background:"#f0fdf9",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",borderBottom:"1.5px solid #e6f7f2"}}>
          {isImage(f.type)
            ? <img src={f.data} alt={f.name} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
            : <div style={{fontSize:40,opacity:.6}}>{icon(f.type)}</div>
          }
        </div>
        {/* info */}
        <div style={{padding:"10px 12px"}}>
          <div style={{fontSize:12,fontWeight:700,color:"#00383f",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginBottom:3}}>{f.name}</div>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
            <Av name={f.brand} size={18}/>
            <span style={{fontSize:11,color:"#64748b",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{f.brand}</span>
          </div>
          <div style={{display:"flex",gap:6}}>
            <a href={f.data} download={f.name} onClick={e=>e.stopPropagation()} style={{fontSize:11,color:"#02a576",fontWeight:600,textDecoration:"none",background:"#e6f7f2",borderRadius:6,padding:"3px 8px"}}>⬇️ Свали</a>
            <button onClick={()=>onOpen(f.item)} style={{fontSize:11,color:"#00383f",fontWeight:600,background:"#f0fdf9",border:"none",borderRadius:6,padding:"3px 8px",cursor:"pointer",fontFamily:"inherit"}}>👁️ Виж</button>
          </div>
        </div>
      </div>)}
    </div>
    ...
    {/* Preview modal */}
    {preview&&<div style={{position:"fixed",inset:0,background:"rgba(0,56,63,.7)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(6px)"}} onClick={()=>setPreview(null)}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:16,maxWidth:800,width:"100%",maxHeight:"90vh",overflow:"hidden",boxShadow:"0 24px 60px rgba(0,0,0,.3)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 18px",borderBottom:"1.5px solid #e6f7f2"}}>
          <div>
            <div style={{fontSize:14,fontWeight:700,color:"#00383f"}}>{preview.name}</div>
            <div style={{fontSize:12,color:"#64748b"}}>{preview.brand}</div>
          </div>
          <div style={{display:"flex",gap:8}}>
            <a href={preview.data} download={preview.name} style={{padding:"7px 14px",borderRadius:8,background:"#02a576",color:"#fff",fontSize:12,fontWeight:600,textDecoration:"none"}}>⬇️ Свали</a>
            <button onClick={()=>setPreview(null)} style={{width:34,height:34,borderRadius:8,border:"1.5px solid #d1fae5",background:"#f8fffe",cursor:"pointer",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",background:"#f0fdf9",minHeight:300,maxHeight:"70vh",overflow:"auto"}}>
          {isImage(preview.type)
            ? <img src={preview.data} alt={preview.name} style={{maxWidth:"100%",maxHeight:"70vh",objectFit:"contain"}}/>
            : isPdf(preview.type)
            ? <iframe src={preview.data} style={{width:"100%",height:"60vh",border:"none"}} title={preview.name}/>
            : <div style={{textAlign:"center",padding:40,color:"#64748b"}}><div style={{fontSize:60,marginBottom:12}}>{icon(preview.type)}</div><div style={{fontSize:14}}>Преглед не е наличен за този тип файл</div><a href={preview.data} download={preview.name} style={{display:"inline-block",marginTop:12,padding:"8px 18px",background:"#02a576",color:"#fff",borderRadius:8,fontSize:13,fontWeight:600,textDecoration:"none"}}>⬇️ Свали файла</a></div>
          }
        </div>
      </div>
    </div>}
...
export default function App(){
  ...
  useEffect(()=>{
    try{
      const r=localStorage.getItem("dm_collab_v6");
      const rs=localStorage.getItem("dm_statuses_v1");
      if(r) setItems(JSON.parse(r)); else setItems(SEED);
      if(rs) setStatuses(JSON.parse(rs));
    }catch{setItems(SEED);}
    setLoading(false);
  },[])

  const persist=(next,sts)=>{
    if(next){ setItems(next); try{ localStorage.setItem("dm_collab_v6",JSON.stringify(next)); }catch{} }
    if(sts){ setStatuses(sts); try{ localStorage.setItem("dm_statuses_v1",JSON.stringify(sts)); }catch{} }
  };

  const handleSave=form=>{
    const now=nowStr(); let next;
    if(form.id){
      ...
      next=items.map(i=>i.id===form.id?{...form,history:hist,comments:old.comments||[],invoices:old.invoices||[]}:i);
      notify("✅ Записано");
    } else {
      next=[...items,{...form,id:Date.now().toString(),dateAdded:new Date().toISOString().slice(0,10),history:[{user:form.assignee||"Екипът",action:"създаде записа",at:now}],comments:[],invoices:[],archived:false}];
      notify("✅ Добавено");
    }
    persist(next,null); setShowForm(false); setEditItem(null);
  };

  const handleDelete=id=>{persist(items.filter(i=>i.id!==id),null); setDetailItem(null); notify("🗑️ Изтрито");};
  const handleArchive=(id,arch)=>{const now=nowStr(); const next=items.map(i=>{if(i.id!==id) return i; return {...i,archived:arch,history:[...(i.history||[]),{user:"Екипът",action:arch?"архивиран":"разархивиран",at:now}]};}); persist(next,null); if(detailItem?.id===id) setDetailItem(next.find(i=>i.id===id)); notify(arch?"📦 Архивирано":"📤 Разархивирано");};
  const handleComment=(id,comment)=>{const next=items.map(i=>{if(i.id!==id) return i; return {...i,comments:[...(i.comments||[]),comment],history:[...(i.history||[]),{user:comment.author,action:`коментира${comment.mentions?.length?` @${comment.mentions.join(", @")}`:""}: "${comment.text.slice(0,50)}"`,at:comment.at}]};}); persist(next,null); if(detailItem?.id===id) setDetailItem(next.find(i=>i.id===id)); notify("💬 Коментарът е добавен");};
  const handleStatusChange=(id,newStatus)=>{const now=nowStr(); const next=items.map(i=>{if(i.id!==id) return i; return {...i,status:newStatus,history:[...(i.history||[]),{user:"Екипът",action:`преместен в "${newStatus}"`,at:now}]};}); persist(next,null); notify(`🔄 ${newStatus}`);};
  const handleInvoiceUpdate=(id,invs)=>{const next=items.map(i=>i.id===id?{...i,invoices:invs}:i); persist(next,null); if(detailItem?.id===id) setDetailItem(next.find(i=>i.id===id));};
  const handleDuplicate=item=>{const now=nowStr(); const copy={...item,id:Date.now().toString(),brand:item.brand+" (копие)",status:"Нов",dateAdded:new Date().toISOString().slice(0,10),history:[{user:"Екипът",action:"дублиран от "+item.brand,at:now}],comments:[],invoices:[],archived:false}; persist([...items,copy],null); setDetailItem(null); notify("⧉ Дублирано");};
  const handleSaveStatuses=sts=>{persist(null,sts); setShowStatusSettings(false); notify("⚙️ Статусите са обновени");};
  ...
  if(loading) return <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:200,fontFamily:"sans-serif",color:"#02a576"}}>Зареждане...</div>;
  ...
  return <div style={{fontFamily:"'DM Sans','Segoe UI',sans-serif",background:"#f0fdf9",minHeight:"100vh",padding:20}}>
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:10}}>
      ...
      <div style={{display:"flex",gap:7,flexWrap:"wrap",alignItems:"center"}}>
        ...
        <button onClick={()=>setShowStatusSettings(true)} title="Настройки статуси" style={{padding:"7px 10px",borderRadius:8,border:"1.5px solid #d1fae5",background:"#fff",color:"#00383f",cursor:"pointer",fontSize:14}}>⚙️</button>
    ...
    {view!=="dashboard"&&view!=="activity"&&view!=="monthly"&&view!=="quarterly"&&view!=="brands"&&view!=="attachments"&&<div style={{background:"#fff",borderRadius:10,padding:"10px 13px",marginBottom:12,border:"1.5px solid #d1fae5",display:"flex",flexWrap:"wrap",gap:7,alignItems:"center"}}>
      ...
      <button onClick={()=>setShowArchive(a=>!a)} style={{...ss,background:showArchive?"#00383f":"#fff",color:showArchive?"#fff":"#00383f",border:"1.5px solid #d1fae5",cursor:"pointer"}}>📦 {showArchive?"Покажи активни":"Покажи архив"}</button>
